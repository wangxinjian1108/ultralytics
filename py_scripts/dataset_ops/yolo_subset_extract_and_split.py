...
def main():
    ...
    parser.add_argument('--val_dataset_ratio', type=float, default=0.1, help='Ratio of validation dataset')
    ...
    # 创建验证集 - 从训练集中随机抽取10%的数据
    print("Creating validation set (10% of training data)...")
    create_validation_set(args.output_dir, val_ratio=args.val_dataset_ratio)
...
def create_validation_set(dataset_dir, val_ratio=0.1):
    """
    Creates a validation set by moving val_ratio% of training images and their labels to val folders.
    ...
    # Log file for validation split
    val_log_file = os.path.join(dataset_dir, 'logs', 'validation_split.txt')
    
    with open(val_log_file, 'w') as log:
        log.write(f"Moving {num_val} images from train to validation set\n\n")
        ...
        for img_path in val_images:
            ...
            try:
                ...
                if os.path.exists(src_label):
                    ...
                    log.write(f"Moved {img_name} and its label to validation set\n")
                else:
                    log.write(f"WARNING: Label file for {img_name} not found\n")
            except Exception as e:
                log.write(f"ERROR moving {img_name}: {str(e)}\n")
    
    print(f"Validation set created with {num_val} images.")
    print(f"Validation split details logged to {val_log_file}")
